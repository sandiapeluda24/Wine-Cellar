...
if (!in_array($rol, ['coleccionista', 'sommelier'])) $errors[] = "Rol inválido";

// Si hay errores, volver al formulario
if (!empty($errors)) {
    $_SESSION['error'] = implode('<br>', $errors);
    header('Location: ../pages/register.php'); // <- (antes tenías ./pages/... y eso falla)
    exit;
}

try {
    $db = getDB(); // <- usa la conexión central (Docker/XAMPP)
